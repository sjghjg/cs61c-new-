#include "compute.h"

// Computes the convolution of two matrices
int convolve(matrix_t *a_matrix, matrix_t *b_matrix, matrix_t **output_matrix) {
  // TODO: convolve matrix a and matrix b, and store the resulting matrix in
  // output_matrix
  // flipped matrix b
  int b_rows = b_matrix->rows;
  int b_cols = b_matrix->cols;
  int a_rows = a_matrix->rows;
  int a_cols = a_matrix->cols;
  int32_t* data_bmatrix = b_matrix->data;
  int32_t amount_bmatrix = b_cols * b_rows;
  int32_t flipped_bdata[amount_bmatrix];
  for (int i = 0; i < amount_bmatrix; i++){
    flipped_bdata[i] = data_bmatrix[amount_bmatrix - i];
  }

  uint32_t output_rows =  a_rows - b_rows +1;
  uint32_t output_cols = a_cols - b_cols +1;

  *output_matrix = malloc(sizeof(matrix_t));
  if (*output_matrix == NULL){
    return -1;
  }

  (*output_matrix)->rows = output_rows;
  (*output_matrix)->cols = output_cols;
  (*output_matrix)->data = malloc(sizeof(int32_t) * output_rows * output_cols);
  if ((*output_matrix)->data == NULL){
    return -1;
  }

  for(uint32_t out_r = 0; out_r< output_rows; out_r++){
    for (uint32_t out_c = 0; out_c< output_cols; out_c++){
      uint32_t sum = 0;
      for (uint32_t x = 0; x < b_rows; x++){
        for (uint32_t y = 0; y < b_cols; y++){
          uint32_t a_r = y + out_r;
          uint32_t a_c = x + out_c;
          int32_t a_val = a_matrix->data[a_r * a_cols + a_c];

          int32_t b_val = flipped_bdata[x * b_cols + y];

          sum += b_val * a_val;
        }
      }
      (*output_matrix)->data[out_r * out_c + out_c] = sum;
    }
  }
  return 0;
}

// Executes a task
int execute_task(task_t *task) {
  matrix_t *a_matrix, *b_matrix, *output_matrix;

  char *a_matrix_path = get_a_matrix_path(task);
  if (read_matrix(a_matrix_path, &a_matrix)) {
    printf("Error reading matrix from %s\n", a_matrix_path);
    return -1;
  }
  free(a_matrix_path);

  char *b_matrix_path = get_b_matrix_path(task);
  if (read_matrix(b_matrix_path, &b_matrix)) {
    printf("Error reading matrix from %s\n", b_matrix_path);
    return -1;
  }
  free(b_matrix_path);

  if (convolve(a_matrix, b_matrix, &output_matrix)) {
    printf("convolve returned a non-zero integer\n");
    return -1;
  }

  char *output_matrix_path = get_output_matrix_path(task);
  if (write_matrix(output_matrix_path, output_matrix)) {
    printf("Error writing matrix to %s\n", output_matrix_path);
    return -1;
  }
  free(output_matrix_path);

  free(a_matrix->data);
  free(b_matrix->data);
  free(output_matrix->data);
  free(a_matrix);
  free(b_matrix);
  free(output_matrix);
  return 0;
}
